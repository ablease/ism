...
var _ = Describe("Platform Controller", func() {
	...
	When("a platform resource is created", func() {
		It("calls the reconcile function", func() {
			instance := &osbapiv1alpha1.Platform{ObjectMeta: metav1.ObjectMeta{Name: "platform-1", Namespace: "default"}}
			Expect(mgrClient.Create(context.TODO(), instance)).To(Succeed())

			Eventually(reconcileRequests).Should(Receive(Equal(
				reconcile.Request{NamespacedName: types.NamespacedName{Name: "platform-1", Namespace: "default"}},
			)))
		})
	})

	Describe("Reconcile", func() {
		//TODO: Add an integration test running against "kinda real" kube and mock http server for platform.
	})
})
